...
const createSupplierCollection = (req, res) => {
  ...
  const getSupplierCollection = (req, res) => {
    ...
    db.query(query, [supplier_id], (err, results) => {
      ...
      if (results.length === 0) {
        res.status(404).json({ error: 'Supplier collection not found for the given supplier ID' });
        return;
      }
  ...
  const updateDetails = (req, res) => {
    ...
    db.query(query, [firstname, lastname, address, telephone, nic, supplier_id], (err, result) => {
      ...
      res.status(200).json({ message: 'Supplier details updated successfully' });
    });
  };
  
  
export { getSupplier, createSupplierCollection, getUser, getAllSuppliers, getSupplierCollection, updateAbout, updateDetails };
