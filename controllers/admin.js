    ...
    (err, result) => {
      ...
      db.query(
        ...
        `;
          ...
          db.query(
            updateMaterialQuery,
            [material_quantity, material_id],
            (updateMaterialErr) => {
              if (updateMaterialErr) {
                console.error(
                  "Error updating material stock:",
                  updateMaterialErr
                );
                return res
                  .status(500)
                  .json({ error: "Error updating material stock" });
              }

              return res
                .status(200)
                .json({ message: "Bailing accepted successfully" });
            }
          );
        }
      );
...
const updateAbout = (req, res) => {
  ...
  const query = "UPDATE admins SET about =? WHERE user_id = ?";
...
const updateDetails = (req, res) => {
  ...
  db.query(
    query,
    [firstname, lastname, address, telephone, nic, admin_id],
    (err, result) => {
      if (err) {
